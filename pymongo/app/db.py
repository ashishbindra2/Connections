...
class ContextManager:
    ...
    def get_filled_slots(self):
        ...
        # print(response)
        filled_slots = None
        # print(self.chat_id, response.__dict__)
    ...
    def add_flow_user(self, ref):
        ...
        try:
            for d in data['flow']:
                ref.add_flows(d)
            print('*' * 60)
            print("data added prompt for new client id is ", self.client_id)
            print('*' * 60)
...
class UserQuery:
    ...
    def add_answers(self, question_no, answers):
        """
            ...
            :return: None
            """
        data = self.get_answers(question_no)
        print(data, "data", len(data))
        if data:
            self.request_api.delete_many({"question_no": question_no})
